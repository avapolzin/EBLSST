...
def plotHist(histAll, histObs, histRec, bin_edges, histAllOD, histObsOD, histRecOD, bin_edgesOD, xtitle, fname):
	c1 = '#0294A5'  #turqoise
	...
	#these are shifted by 1 incorrectly
	histAll = np.insert(histAll, 0, 0)[0:-1]
	...
	ax1.set_xlim(min(bin_edges),max(bin_edges))
	...
	ax3.set_xlim(min(bin_edges),max(bin_edges))
